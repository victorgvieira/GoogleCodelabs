...
//DONE Create the ContentProvider class
...
class LogsContentProvider : ContentProvider() {
    ...
    // DONE Create an entryPoint interface
    ...
    // DONE Create a method to access a value from an entryPoint
    // NOTE: the Context must match the same type in the get method from EntryPointAccessors
    ...
    override fun query(
        ...
        return when (code) {
            CODE_LOGS_DIR, CODE_LOGS_ITEM -> {
                val appContext = context?.applicationContext ?: throw IllegalStateException()
                val logDao: LogDao = getLogDao(appContext)

                val cursor: Cursor? = when (code) {
                    CODE_LOGS_DIR -> logDao.selectAllLogsCursor()
                    else -> logDao.selectLogById(ContentUris.parseId(uri))
                }
